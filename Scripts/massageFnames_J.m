...
%       the first number in the name. If that first number was not a 2 it
%       would add a "20" behind the numbers. 
%       This worked well for anything after 1999 and before 2020 but needed
%       to be fixed to still correct for 2020 2 digit years in file names.
%
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
fid=fopen([path2research,'ICE/ICETHICKNESS/Data/Stage_of_development/Hemispheric/fnamesITcor.txt'],'w');
for ii=1:length(fnames)
    fprintf(fid,'%s\n',fnames{ii,:});
end
fclose(fid);
...
fnames=textread([path2research,'ICE/ICETHICKNESS/Data/Stage_of_development/Hemispheric/fnamesITcor.txt'],'%s');
dummyfn=char(fnames);
...
fid=fopen([path2research,'ICE/ICETHICKNESS/Data/Stage_of_development/Hemispheric/fnamesITsortedcor.txt'],'w');
for ii=1:length(fnames)
    fprintf(fid,'%s\n',fnames{ii,:});
end
fclose(fid);
