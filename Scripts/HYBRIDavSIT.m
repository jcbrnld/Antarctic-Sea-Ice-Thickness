...
% Same as chaper 2 from SITworkingscript_Final.m 
% BUT ***
% Partial concentration neighboring year average was replaced with record
% length monthly average. SOD still uses the neighboring year aveerage thus
% making this the "hybrid" method. 

% Partial concentration is missing data for large portions of the early
% thousands, much larger and more continuous chunks of time than SOD. 
% SOD on the other hand is missing data periodically but with no long
% continuous stretch of extreme shortages. THUS the moving average more
% steadily contributes to filling missing values using neighboring year
% data. SIC on the other hand still has to skip large spans of time even in
% the neighboring year system so a more lengthened averaging time is
% appreciated. Additionally this cuts computation time almost in half. 
...
if length(sector)==2
    save(['ICE/ICETHICKNESS/Data/MAT_files/Final/orig_timescale/Sectors/sector',sector,'_hybrid.mat'], 'SIT', '-v7.3');
else
    sloc = strfind(sector, 'SIC');
    save(['ICE/ICETHICKNESS/Data/MAT_files/Final/zones/',sector(1:sloc-2),'_hybrid.mat'], 'SIT', '-v7.3');
end
